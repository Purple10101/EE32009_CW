...
#pk = sig_peak_det(degrade(data1['d'][0], data2['d'][0],0.25))
#val_peak_det(pk, data1['Index'][0])
#sig_peak_det(data2['d'][0])
...
torch.save(model.state_dict(), "src/nn/ind_mdl/models/D2/20251111_neuron_event_det_cnn_window_norm_d2.pt")

# load model and evaluate performance
model = SpikeNet().to(device)
model.load_state_dict(torch.load("src/nn/ind_mdl/models/D2/20251111_neuron_event_det_cnn_window_norm_d2.pt"))
model.eval()
